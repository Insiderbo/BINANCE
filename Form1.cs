...
namespace RSI_test
{
    public partial class Form1 : Form
    {
        ...
        private int slow_level = 15;
        private int main_level = 25;
        private int drop_price = 20;
        public Form1()
        {
            InitializeComponent();
        }
        ...
        {
            candles[pair] = await LoadCandles(pair, candle_period, 22);
        ...
        private void UpdateRow(string pair)
        {
            ...
            row.Cells[11].Value = Profit(
                candles_for_pair.Select(x => x.highPrice).ToList(),
                candles_for_pair.Select(x => x.lowPrice).ToList(),
                candles_for_pair.Select(x => x.сlosePrice).ToList(),
                candles[pair].Last());
            row.Cells[12].Value = VolumeLast(candles[pair].Last());
            row.Cells[13].Value = GlobalAvg(
                candles_for_pair.Select(x => x.highPrice).ToList(),
                candles_for_pair.Select(x => x.lowPrice).ToList());
            row.Cells[14].Value = DifferenceVolume(
               candles_for_pair.Skip(candles_for_pair.Count - 3).Select(x => x.volume).ToList(),
               candles_for_pair.Skip(candles_for_pair.Count - 3).Select(x => x.сlosePrice).ToList(),
               candles[pair].Last());
            PaintRow(row);
        }
       
        private object DifferenceVolume(List<double> volume, List<double> closePrice, Candle candle)
        {
            var yestvol = Math.Round(volume.ElementAt(1)*closePrice.ElementAt(1),2);
            var lastvol = Math.Round(candle.volume*candle.сlosePrice,2);
            return Math.Round((yestvol/lastvol-1)*100,2);
        }

        private object GlobalAvg(List<double> highPrice, List<double> lowPrice)
        {
            return Math.Round((highPrice.Max() + lowPrice.Min()) / 2, 8);
        }

        private object VolumeLast(Candle candle)
        {
            return candle.volume * candle.сlosePrice;
        }

        private object Profit(List<double> highPrices, List<double> closePrices, List<double> LowPrices, Candle candle)
        {
            return Math.Round((100 / candle.сlosePrice) * ((highPrices.Max() + LowPrices.Min()) / 2), 2);
        }
        ...
        private double CalculateGlobalPower(double rsislow_calculators, double rsisnormal_calculators)
        {
            return Math.Round((rsislow_calculators + rsisnormal_calculators) / 2, 2);
        }

        private double CalculateMomentum(List<double> closePrices)
        {
            return (closePrices.Last() * 100) / closePrices.ElementAt(0);
        }
        ...
        {
            return Math.Round(((highPrices.Max() - closePrices.Last()) / highPrices.Max()) * 100);
        }
        ...
        void PaintRow(DataGridViewRow r)
        {
            ...
            if (SortingAlgorithm(r))
            {
                ...
                if (r.DefaultCellStyle.BackColor == Color.LavenderBlush && double.Parse(r.Cells[1].Value.ToString()) > double.Parse(r.Cells[2].Value.ToString())
                    && DataGridViewCheckBoxCell.Equals(r.Cells[15].Value, null) == false && checkBoxBuy.Checked == false)
                {
                    NotifyTg(r.Cells[0].Value.ToString() + "  Buy price: " + r.Cells[8].Value.ToString() + " Period " + candle_period);
                }
                if (r.DefaultCellStyle.BackColor == Color.LavenderBlush && double.Parse(r.Cells[1].Value.ToString()) > double.Parse(r.Cells[2].Value.ToString())
                    && DataGridViewCheckBoxCell.Equals(r.Cells[15].Value, null) == true && checkBoxBuy.Checked == true)
                {
                    NotifyTg(r.Cells[0].Value.ToString() + "  Buy price: " + r.Cells[8].Value.ToString() + " Period " + candle_period);
                }
            }
        ...
        {
            for (int i = 1; i < r.Cells.Count-1; i++)
            {
                if (!double.TryParse(r.Cells[i].Value.ToString(), out var d) && checkBoxBuy.Checked == true)
                {
                    return false;
                }
                if (d == 0 && checkBoxBuy.Checked == true)
                {
                    return false;
                }
            }
        ...
        private async void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if(TimeZoneInfo.ConvertTimeToUtc(now).Day < TimeZoneInfo.ConvertTimeToUtc(DateTime.Now).Day)
            {
                comboBox1_SelectedIndexChanged(sender, e);
            }
            timer1.Stop();
            Status($"loaded one candle");
            timer1.Start();
        }
        ...
        {
            var path = @"C:\Users\insiderbo\Documents\Telegramm_Bot\bin\Debug\net5.0\Telegramm_Bot.exe";
        ...
        {
            dataGridView1.CurrentRow.Cells[15].Value = true;
        }
